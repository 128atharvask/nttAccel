...
class NTTTest_C extends AnyFlatSpec with ChiselScalatestTester {
  "NTT_ctrl_dp" should "output scap0" in {
    test(new NTT_ctrl_dp).withAnnotations(Seq(WriteVcdAnnotation)) { dut =>
      ...
      // Wait for input_ready before proceeding
      println("Waiting for input_ready to go high...")
      while (!dut.io.input_ready.peek().litToBoolean) {
        dut.clock.step(1)
      }
      println("input_ready is high, proceeding with test...")

      // Define the input data for the test (256 values provided by the user)
      val inputData = Seq(
