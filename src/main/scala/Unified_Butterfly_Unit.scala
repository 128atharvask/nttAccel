...
class UnifiedButterflyUnit extends Module {
  ...
  when(io.enable) {
    // Cycle 1
    when(!io.select && !io.inverse) {
      reg1 := Mux(io.u > io.v, io.u - io.v, io.u + 3329.U - io.v)
      reg0 := Mux(io.u + io.v > 3329.U, io.u + io.v - 3329.U, io.u + io.v)
    }.otherwise {
      reg0 := io.u
      reg1 := io.v
    }

    reg2 := Mux(io.inverse, 3303.U, io.w)
    str_del := io.start
    ...
    when(!io.select) {
      ...
      y_reg := Mux(reg_del5 > c3, reg_del5 - c3, reg_del5 + 3329.U - c3)
      x_reg := Mux(reg_del5 + c3 > 3329.U, reg_del5 + c3 - 3329.U, reg_del5 + c3)
    }
  }
